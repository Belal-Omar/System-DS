<?php
// ملف: profile_merchant.php - الملف الشخصي للتاجر
session_start();
include("config.php");
...
$user['user_type'] = $user['user_type'] ?? 'تاجر';
...
// حساب الإحصائيات للتاجر (من منتجاته فقط)
$total_profit_query = $conn->query("
    SELECT COALESCE(SUM(oi.original_price * oi.quantity), 0) as total 
    FROM orders o
    JOIN order_items oi ON o.id = oi.order_id
    JOIN products p ON oi.product_id = p.id
    WHERE p.user_id = $user_id AND o.status IN ('تم التوصيل', 'محصل', 'مكتمل')
");
$total_profit = $total_profit_query ? floatval($total_profit_query->fetch_assoc()['total'] ?? 0) : 0;
...
");
$withdrawn = $withdrawn_query ? floatval($withdrawn_query->fetch_assoc()['total'] ?? 0) : 0;
...
$available_balance = $total_profit - $withdrawn;
...
<head>
    ...
    <title>الملف الشخصي - التاجر</title>
...
<body class="bg-gray-50" style="font-family: 'Cairo', sans-serif; background: linear-gradient(135deg, #f8fafc, #eef2f3);">
    <?php
    // Navbar مباشرة في الصفحة
    ?>
    <style>
        .main-navbar {
            position: fixed;
    ...
    <nav class="main-navbar" id="mainNavbar">
        <div style="display: flex; align-items: center; gap: 2rem;">
            <a href="home111.php" class="navbar-logo">لوحة التاجر</a>
            <div class="navbar-links">
                <a href="home111.php" class="navbar-link">الرئيسية</a>
                <a href="products1.php" class="navbar-link">المنتجات</a>
                <a href="orders1.php" class="navbar-link">الطلبات</a>
                <a href="withdrawing1.php" class="navbar-link">السحب</a>
            </div>
        </div>
        <div class="navbar-actions">
            <?php if ($user_id && $user): ?>
                ...
                <a href="logout.php" class="navbar-btn-logout">تسجيل الخروج</a>
            <?php endif; ?>
        </div>
    </nav>
    <script>
    ...
    <div class="container mx-auto px-4 py-8">
        ...
        <h1 class="text-3xl font-bold text-gray-800 mb-8 text-center">الملف الشخصي - التاجر</h1>
                ...
                <div class="bg-white rounded-lg shadow-sm p-6 mb-6">
                    ...
                    <div class="space-y-4">
                        ...
                        <div class="flex justify-between items-center">
                            ...
                            <span class="font-bold text-blue-600"><?php echo number_format($total_profit, 2); ?> د.ل</span>
                        </div>
                        
                        <div class="flex justify-between items-center">
                            <span class="text-gray-600">إجمالي المسحوب</span>
                            <span class="font-bold text-purple-600"><?php echo number_format($withdrawn, 2); ?> د.ل</span>
                        </div>
                        ...
                        <div class="flex justify-between items-center">
                            ...
                            <span class="font-bold text-indigo-600"><?php echo $total_products; ?></span>
                        </div>
                    </div>
                </div>

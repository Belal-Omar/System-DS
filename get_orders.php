...
try {
    ...
    // بناء استعلام الطلبات - إذا كان المستخدم مسجل دخول، جلب طلباته فقط
    // إذا لم يكن مسجل دخول، جلب جميع الطلبات (للعرض العام)
    ...
    // إذا لم يكن هناك user_id، نترك where_clause فارغاً لجلب جميع الطلبات
    
    // استعلام محسن لجلب الطلبات
    $result = $conn->query("
        SELECT 
            o.id,
